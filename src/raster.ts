import type { RasterRow } from './encode.js';

/**
 * 1-bit cutoff. A pixel at or above this luma leaves the stock bare, below it
 * the head fires.
 *
 * Deliberately the same value and the same formula as yplib's: a toolchain
 * that drives both printers must threshold identically, or the same design
 * previews one way and prints two different ways.
 */
 ...
 * function, or a preview and the paper silently disagree. Alpha is composited
 * against white because unburned stock is what shows through, so a
...
/**
 * Reduce RGBA pixels to rows of dots: 1 where the head fires, 0 where it does
 * not. Row length is the source width.
 *
